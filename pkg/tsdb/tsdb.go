...
/* func request(client remote.WriteClient, ctx context.Context, req []byte) error {
	compressed := snappy.Encode(nil, req)

	// compress the snappy-compressed data using gzip
	...
	return client.Store(ctx, buf.Bytes())
}
*/
/*
	 func (pr *PromRemote) Send(value float64, newLabels []promremote.Label) {
		now := time.Now()
		timeSeriesList := promremote.TimeSeries{
			Labels: append(pr.DefaultLabels, newLabels...),
			Datapoint: promremote.Datapoint{
				Timestamp: now,
				Value:     value,
			},
		}
		tsList := promremote.TSList{timeSeriesList}
		cc := context.Background()
		ctx, cancel := context.WithTimeout(cc, time.Minute)
		level.Debug(pr.l).Log("发送内容,value", value, "list", tsList)
		defer cancel()

		err := request(pr.Client, pr.l, ctx, tsList, pr.UA)

		if err != nil {
			level.Warn(pr.l).Log("发送请求失败", err, "进入重试", "10second")
			nt := time.NewTicker(time.Second * 10)
			...
			for {
				...
				case <-nt.C:
					err = request(pr.Client, pr.l, ctx, tsList, pr.UA)
					level.Debug(pr.l).Log("重试发送", "10 seconds", "list", tsList)
					if err == nil {
						level.Info(pr.l).Log("重试发送", "成功", "list", tsList)
						return
					}
				case <-ctx.Done():
					level.Error(pr.l).Log("发送请求错误", "超时", "list", tsList)
					return
				}
			}
		}
		level.Debug(pr.l).Log("发送tsdb", "成功", "list", tsList)
	}
*/
/* func request(client promremote.Client, l log.Logger, ctx context.Context, tsList promremote.TSList, UA string) error {
	response, err := client.WriteTimeSeries(ctx, tsList,
		promremote.WriteOptions{
			Headers: map[string]string{"User-Agent": UA}})

	if err != nil {
		level.Error(l).Log("push error", err.Error(), "err code", err.StatusCode(), "result status", response.StatusCode)
		return err
	}
